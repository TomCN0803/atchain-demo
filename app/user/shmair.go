...
func Share_f(user *User,contract *client.Contract,x int64 ,n int,p *big.Int,publicKey []*paillier.PublicKey,args []string) {
	...
	for i := uint64(0); i < t; i++ {
		...
		a1 := (a.Val).String()
		err :=user.SubmitTransaction(contract, "Insert",args[i] ,a1)
		if err != nil {
			fmt.Println(err)
			fmt.Println("-1")
		}
